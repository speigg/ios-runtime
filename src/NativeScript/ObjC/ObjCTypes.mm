...
JSValue toValue(ExecState* execState, id object, Structure* (^structureResolver)()) {
    ...
    ObjCWrapperObject* wrapper = ObjCWrapperObject::create(execState->vm(), structureResolver(), object, globalObject);
    globalObject->interop()->objectMap().set(object, wrapper);
    return wrapper;
}
}
